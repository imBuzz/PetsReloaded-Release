...
import me.imbuzz.dev.petsreloaded.nms.v1_17_R1.entities.NMSPetEntity;
...
public class EntityHandler implements IEntityHandler {
    ...
    public void updatePetStatus(Entity entity, PetStatus... petStatuses) {
        ...
        for (PetStatus petStatus : petStatuses) {
            switch (petStatus) {
                case NO_AI: {
                    ((EntityInsentient) nmsEntity).bP = new PathfinderGoalSelector(nmsEntity.getWorld().getMethodProfilerSupplier());
                    ((EntityInsentient) nmsEntity).bP
                            .a(100, new PathfinderGoalLookAtPlayer((EntityInsentient) nmsEntity, EntityHuman.class, 8.0F));

                    break;
                }
                case SILENT: {
                    ((CraftEntity) entity).setSilent(true);
                    break;
                }
                case INVULNERABLE: {
