package me.imbuzz.dev.petsreloaded.nms.v1_10_R1.entities;
...
import me.imbuzz.dev.petsreloaded.nms.v1_10_R1.goals.PathFinderGoalPetsBasic;
...
public class NMSPetEntity extends EntitySheep {
    ...
    public void g(float sideMot, float forMot) {
        if (!passengers.isEmpty()) {
            net.minecraft.server.v1_10_R1.Entity passenger = passengers.get(0);

            this.lastYaw = this.yaw = passenger.yaw;
            this.pitch = (passenger.pitch * 0.5F);
            setYawPitch(this.yaw, this.pitch);
            this.aQ = this.aK = this.yaw;

            sideMot = (((EntityLiving) passenger).bf * ((float) pet.getSettings().getSpeed() / 7));
            forMot = ((EntityLiving) passenger).bg * ((float) pet.getSettings().getSpeed() / 7);
            if (forMot <= 0.0F) {
            ...
            if (this.onGround) {
                try {
                    Field jump = EntityLiving.class.getDeclaredField("be");
            ...
            this.aG = this.aH;//Some extra things
